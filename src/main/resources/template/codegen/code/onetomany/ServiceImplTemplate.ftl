...
import ${packageName}<#if moduleName?exists><#if moduleName!=''>.${moduleName}</#if></#if>.entity.${schedule.className?cap_first}Entity;
...
public class ${entityName?cap_first}ServiceImpl  extends CommonServiceImpl<${entityName?cap_first}Entity> implements  I${entityName?cap_first}Service {
	<#list schedules as schedule>
	@Autowired
	...
	@Override
	public void save(${entityName?cap_first}Entity ${entityName?uncap_first}) {
		// 保存主表
		...
		List<${schedule.className?cap_first}Entity> ${schedule.className?uncap_first}List = JSONObject.parseArray(${schedule.className?uncap_first}ListJson, ${schedule.className?cap_first}Entity.class);
		for (${schedule.className?cap_first}Entity ${schedule.className?uncap_first} : ${schedule.className?uncap_first}List) {
			// 保存字段列表
	...
	@Override
	public void update(${entityName?cap_first}Entity ${entityName?uncap_first}) {
		try {
		    <#list schedules as schedule>
			// 获得以前的数据
			List<${schedule.className?cap_first}Entity> old${schedule.className?cap_first}List = ${schedule.className?uncap_first}Service.list("${schedule.parentField?uncap_first}", ${entityName?uncap_first});
			// 字段
			String ${schedule.className?uncap_first}ListJson = StringEscapeUtils
					.unescapeHtml4(ServletUtils.getRequest().getParameter("${schedule.className?uncap_first}ListJson"));
			List<${schedule.className?cap_first}Entity> ${schedule.className?uncap_first}List = JSONObject.parseArray(${schedule.className?uncap_first}ListJson,
					${schedule.className?cap_first}Entity.class);
			</#list>
			// 更新主表
			super.update(${entityName?uncap_first});
			<#list schedules as schedule>
			${schedule.className?uncap_first}List = JSONObject.parseArray(${schedule.className?uncap_first}ListJson, ${schedule.className?cap_first}Entity.class);
			...
			for (${schedule.className?cap_first}Entity ${schedule.className?uncap_first} : ${schedule.className?uncap_first}List) {
				...
				if (StringUtils.isEmpty(${schedule.className?uncap_first}.getId())) {
					...
					${schedule.className?cap_first}Entity oldTicket = ${schedule.className?uncap_first}Service.get(${schedule.className?uncap_first}.getId());
			...
			// 删除老数据
			for (${schedule.className?cap_first}Entity ${schedule.className?uncap_first} : old${schedule.className?cap_first}List) {
