...
public class IncludeSqlDirective implements TemplateDirectiveModel {
	...
	public void execute(Environment env, @SuppressWarnings("rawtypes") Map params, TemplateModel[] loopVars,
		...
		if (params.get("refid") != null) {
			...
			String templateKey = Utils.getCacheKeyByType(statementId, StatementType.SQL);
			statement = ehcacheUtil.getString(templateKey);
			if (StringUtils.isEmpty(statement)) {
				throw new DynamicException("请检查" + statementId + "的SQL是否已经定义");
			}
			env.include(new Template(statementId, new StringReader(statement), env.getConfiguration()));
		} else {
			throw new DynamicException("SQL include引用的ID未定义");
		}
		env.getOut().write(" ");//保证导入后两边空格
	}
}