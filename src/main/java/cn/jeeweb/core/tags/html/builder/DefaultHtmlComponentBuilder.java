...
public class DefaultHtmlComponentBuilder implements HtmlComponentBuilder, ResourceLoaderAware {
	...
	private void buildMap(Resource resource) {
		...
		try {
			...
			if (isHtmlComponentXml(metadataXml)) {
	...
	private static boolean isHtmlComponentXml(XmlDocument xmlDocument) {
		return "html-component".equals(xmlDocument.getDocumentTree().getRootElement().getName());
	}
